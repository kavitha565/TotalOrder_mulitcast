...
public class MaintainOrder2 {
	...
	public static boolean canWeSendAckToThisGuy(Event event){
		...
		if(!acknowledgementBuffer.isEmpty() && acknowledgementBuffer.containsKey(event.getEventId())){
			if(acknowledgementBuffer.get(event.getEventId()).contains(MaintainOrder2.processId)){
				return false;
			}
		}
	...
	public static void main(String[] args) {
		MaintainOrder2 totalOrder = new MaintainOrder2();
		totalOrder.init();

		try {
			if(MaintainOrder2.createConnectionsThread != null){
				MaintainOrder2.createConnectionsThread.join();
			}
			if(MaintainOrder2.orderedDeliveryThread != null){
				MaintainOrder2.orderedDeliveryThread.join();
			}
			if(MaintainOrder2.eventAcknowledgementThread != null){
				MaintainOrder2.eventAcknowledgementThread.join();
			}
		...
		System.out.println("Process "+MaintainOrder2.processId+" ended!");
