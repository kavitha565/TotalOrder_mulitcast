...
public class MaintainOrder3 {
	public static final String processId = "P2";
	public static final int processNum = 2;
	...
	public static boolean canWeSendAckToThisGuy(Event event){
		...
		if(!acknowledgementBuffer.isEmpty() && acknowledgementBuffer.containsKey(event.getEventId())){
			if(acknowledgementBuffer.get(event.getEventId()).contains(MaintainOrder3.processId)){
				return false;
			}
		}
	...
	public static void main(String[] args) {
		MaintainOrder3 totalOrder = new MaintainOrder3();
		totalOrder.init();

		try {
			if(MaintainOrder3.createConnectionsThread != null){
				MaintainOrder3.createConnectionsThread.join();
			}
			if(MaintainOrder3.orderedDeliveryThread != null){
				MaintainOrder3.orderedDeliveryThread.join();
			}
			if(MaintainOrder3.eventAcknowledgementThread != null){
				MaintainOrder3.eventAcknowledgementThread.join();
			}
		...
		System.out.println("Process "+MaintainOrder3.processId+" ended!");
